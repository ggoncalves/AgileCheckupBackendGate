...
class CompanyRequestHandlerTest {
  ...
  void shouldSuccessfullyCreateCompanyWithRequiredFields() {
    ...
    APIGatewayProxyRequestEvent request = new APIGatewayProxyRequestEvent().withPath("/companies").withHttpMethod("POST").withBody(requestBody);

    Company createdCompany = Company.builder().id("new-company-id").documentNumber("12345678000123").name("Tech Company Inc.").email("contact@techcompany.com").description("A technology company").tenantId("tenant-123").size(CompanySize.STARTUP).industry(Industry.TECHNOLOGY).build();

    doReturn(Optional.of(createdCompany)).when(companyService).create(
        anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), isNull(), isNull(), any(), any());
    ...
    verify(companyService).create(
        eq("12345678000123"), eq("Tech Company Inc."), eq("contact@techcompany.com"), eq("A technology company"), eq("tenant-123"), eq(CompanySize.STARTUP), eq(Industry.TECHNOLOGY), isNull(), isNull(), isNull(), isNull()
    );
  ...
  void shouldSuccessfullyCreateCompanyWithAllFields() {
    ...
    Company createdCompany = Company.builder().id("full-company-id").documentNumber("98765432000198").name("Full Company Ltd.").email("info@fullcompany.com").description("A complete company").tenantId("tenant-456").size(CompanySize.LARGE).industry(Industry.FINANCE).website("https://www.fullcompany.com").legalName("Full Company Legal Ltd.").build();

    ArgumentCaptor<NaturalPerson> contactPersonCaptor = ArgumentCaptor.forClass(NaturalPerson.class);
    ArgumentCaptor<Address> addressCaptor = ArgumentCaptor.forClass(Address.class);

    doReturn(Optional.of(createdCompany)).when(companyService).create(
        anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), anyString(), anyString(), any(NaturalPerson.class), any(Address.class));
    ...
    // Then
    verify(companyService).create(
        eq("98765432000198"), eq("Full Company Ltd."), eq("info@fullcompany.com"), eq("A complete company"), eq("tenant-456"), eq(CompanySize.LARGE), eq(Industry.FINANCE), eq("https://www.fullcompany.com"), eq("Full Company Legal Ltd."), contactPersonCaptor.capture(), addressCaptor.capture()
    );
  ...
  void shouldReturnBadRequestWhenRequiredFieldsMissing() {
    ...
    verify(companyService, never()).create(anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), anyString(), anyString(), any(), any());
  }
...
//        "    \"lastName\": \"Doe\",\n" +
    ...
    APIGatewayProxyRequestEvent request = new APIGatewayProxyRequestEvent().withHttpMethod("PUT").withPath("/companies/" + companyId).withBody(requestBody);
...
//    contactPerson.setLastName("Doe");
    ...
    doReturn(Optional.of(existingCompany)).when(companyService).update(
        anyString(), anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), any(), any(), any(), any());
    ...
    verify(companyService).update(
        eq(companyId), eq("11111111000111"), eq("Updated Company"), eq("updated@company.com"), eq("Updated description"), eq("tenant-789"), eq(CompanySize.MEDIUM), eq(Industry.HEALTHCARE), eq("https://www.updated.com"), eq("Updated Company Legal"), contactPersonCaptor.capture(), addressCaptor.capture()
    );
  ...
  void shouldSuccessfullyUpdateCompanyWithAllFields() {
    ...
    APIGatewayProxyRequestEvent request = new APIGatewayProxyRequestEvent().withPath("/companies/" + companyId).withHttpMethod("PUT").withBody(requestBody);

    Company updatedCompany = Company.builder().id(companyId).documentNumber("11111111000111").name("Updated Company").email("updated@company.com").description("Updated description").tenantId("tenant-789").size(CompanySize.MEDIUM).industry(Industry.HEALTHCARE).website("https://www.updated.com").legalName("Updated Company Legal").build();

    doReturn(Optional.of(updatedCompany)).when(companyService).update(
        anyString(), anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), anyString(), anyString(), any(), any());
    ...
    // Then
    verify(companyService).update(
        eq(companyId), eq("11111111000111"), eq("Updated Company"), eq("updated@company.com"), eq("Updated description"), eq("tenant-789"), eq(CompanySize.MEDIUM), eq(Industry.HEALTHCARE), eq("https://www.updated.com"), eq("Updated Company Legal"), isNull(), isNull()
    );
  ...
  void shouldReturnNotFoundWhenUpdatingNonExistentCompany() {
    ...
    doReturn(Optional.empty()).when(companyService).update(
        anyString(), anyString(), anyString(), anyString(), anyString(), anyString(), any(CompanySize.class), any(Industry.class), isNull(), isNull(), any(), any());
  ...
  void shouldSuccessfullyUpdateCompanyWithEmptyGenderFields() {
    ...
    Company updatedCompany = Company.builder().id(companyId).name("New Company without Optional Fields").email("glauciocgoncalves@gmail.com").tenantId("new-company-tenant").documentNumber("74.790.229/0001-03").description("New Company without Optional Fields").size(CompanySize.STARTUP).industry(Industry.NONPROFIT).website("").legalName("").build();
    ...
    ArgumentCaptor<NaturalPerson> contactPersonCaptor = ArgumentCaptor.forClass(NaturalPerson.class);
    verify(companyService).update(
        eq(companyId), eq("74.790.229/0001-03"), eq("New Company without Optional Fields"), eq("glauciocgoncalves@gmail.com"), eq("New Company without Optional Fields"), eq("new-company-tenant"), eq(CompanySize.STARTUP), eq(Industry.NONPROFIT), eq(""), eq(""), contactPersonCaptor.capture(), any());
